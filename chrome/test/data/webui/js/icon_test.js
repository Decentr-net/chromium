...
import {isChromeOS, isLacros, isLinux, isMac, isWindows} from 'decentr://resources/js/cr.m.js';
import {getFavicon, getFaviconForPageURL, getFileIconUrl} from 'decentr://resources/js/icon.js';
...
suite('IconModuleTest', function() {
  test('GetFaviconForPageURL', function() {
    ...
    function getExpectedImageSet(size) {
      const expectedDesktop = '-webkit-image-set(' +
          `url("decentr://favicon2/?size=${size}&scale_factor=1x&page_url=` +
          encodeURIComponent(url) + '&allow_google_server_fallback=0") 1x, ' +
          `url("decentr://favicon2/?size=${size}&scale_factor=2x&page_url=` +
  ...
  test('GetFavicon', function() {
    ...
    const expectedDesktop = '-webkit-image-set(' +
        'url("decentr://favicon2/?size=16&scale_factor=1x&icon_url=' +
        encodeURIComponent('http://foo.com/foo.ico') + '") 1x, ' +
        'url("decentr://favicon2/?size=16&scale_factor=2x&icon_url=' +
  ...
  test('GetFileIconUrl', function() {
    assertEquals(
        ...
        'decentr://fileicon/?path=file+path&scale=' + window.devicePixelRatio +
            'x');
  });
});
