...
var newTabUrls = [
  ...
  'decentr-search://local-ntp/local-ntp.html',
];

function testExecuteScriptInNewTab() {
  // Create a new tab to decentr://newtab and wait for the loading to complete.
    ...
    chrome.tabs.executeScript(tab.id, {file: 'script.js'}, function() {
      ...
      chrome.test.assertTrue(
          lastErrorMessage.indexOf('Cannot access contents of') != -1 ||
              lastErrorMessage.indexOf('Cannot access a decentr:// URL') != -1,
          lastErrorMessage);
      chrome.test.succeed();
    });
  });
  chrome.tabs.create({url: 'decentr://newtab'});
}
