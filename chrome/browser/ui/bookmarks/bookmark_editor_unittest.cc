...
const BookmarkNode* InitBookmarkBar(BookmarkModel* model) {
  ...
  model->AddURL(bookmarkbar, 0, u"url0", GURL("decentr://newtab"));
  model->AddURL(bookmarkbar, 1, u"url1", GURL("decentr://newtab"));
  return bookmarkbar;
}
...
TEST(BookmarkEditorTest, ApplyEditsWithMultipleURLs) {
  ...
  url_data_0.url = absl::make_optional(GURL("decentr://newtab"));
  url_data_0.title = url_title_0;
  BookmarkEditor::EditDetails::BookmarkData url_data_1;
  url_data_1.url = absl::make_optional(GURL("decentr://newtab"));
...
TEST(BookmarkEditorTest, ApplyEditsWithNestedFolder) {
  ...
  url_data.url = absl::make_optional(GURL("decentr://newtab"));
...
TEST(BookmarkEditorTest, ApplyEditsWithURLsAndNestedFolders) {
  ...
  nested_url_data.url = absl::make_optional(GURL("decentr://newtab"));
