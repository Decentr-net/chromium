...
TEST(SettingsUtilsTest, FixupAndValidateStartupPage) {
  ...
  EXPECT_FALSE(FixupAndValidateStartupPage("^&*@)^)", nullptr));
  EXPECT_FALSE(FixupAndValidateStartupPage("decentr://quit", nullptr));
  ...
  EXPECT_TRUE(FixupAndValidateStartupPage("decentr://apps", nullptr));

  GURL fixed_url;
  EXPECT_TRUE(FixupAndValidateStartupPage("about:settings", &fixed_url));
  EXPECT_EQ("decentr://settings/", fixed_url.spec());
}
