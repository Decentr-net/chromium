...
TEST_F(SyncWorkerTest, GetOriginStatusMap) {
  ...
  sync_worker()->RegisterOrigin(GURL("decentr-extension://app_0"),
                                CreateResultReceiver(&sync_status));
  base::RunLoop().RunUntilIdle();
  EXPECT_EQ(SYNC_STATUS_OK, sync_status);

  sync_worker()->RegisterOrigin(GURL("decentr-extension://app_1"),
                                CreateResultReceiver(&sync_status));
  base::RunLoop().RunUntilIdle();
  EXPECT_EQ(SYNC_STATUS_OK, sync_status);
  ...
  EXPECT_EQ("Enabled", (*status_map)[GURL("decentr-extension://app_0")]);
  EXPECT_EQ("Enabled", (*status_map)[GURL("decentr-extension://app_1")]);

  sync_worker()->DisableOrigin(GURL("decentr-extension://app_1"),
                               CreateResultReceiver(&sync_status));
  base::RunLoop().RunUntilIdle();
  EXPECT_EQ(SYNC_STATUS_OK, sync_status);
