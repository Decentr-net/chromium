// Copyright 2019 The Chromium Authors. All rights reserved.
// Use of this source code is governed by a BSD-style license that can be
// found in the LICENSE file.

#include "ash/webui/help_app_ui/url_constants.h"

namespace ash {

const char kChromeUIHelpAppHost[] = "help-app";
const char kChromeUIHelpAppKidsMagazineHost[] = "help-app-kids-magazine";
const char kChromeUIHelpAppURL[] = "decentr://help-app/";
const char kChromeUIHelpAppUntrustedURL[] = "decentr-untrusted://help-app/";
const char kChromeUIHelpAppKidsMagazineUntrustedURL[] =
    "decentr-untrusted://help-app-kids-magazine/";

}  // namespace ash
