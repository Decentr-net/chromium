...
#define SCHEME NCG("http|https|ftp|chrome|decentr-extension|android|rtsp|file")
...
bool IsUrlExempt(re2::StringPiece url,
  ...
  // Check for decentr:// URLs that are exempt.
  if (url.starts_with("decentr://")) {
    // We allow everything in decentr://resources/.
    if (url.starts_with("decentr://resources/"))
      return true;

    // We allow decentr://*/crisper.js.
  ...
  // Exempt URLs of the format decentr-extension://<first-party-id>/*.js
  if (!url.starts_with("decentr-extension://"))
    return false;
  ...
  const re2::StringPiece url_sub =
      url.substr(sizeof("decentr-extension://") - 1);
  while (test_id) {
